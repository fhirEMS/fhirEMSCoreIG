...
| Section | Description | FHIR Mapping |
|---------|-------------|-------------|
...
| NEMSIS Usage | Description | FHIR MS |
|---|---|---|
...
2. VitalGroup repeating groups → multiple `Observation` resources linked by `encounter` and `effectiveDateTime`
3. Timestamps from `eTimes` → `ext-ems-times` complex extension on `Encounter`
4. NEMSIS coded values → FHIR `CodeableConcept` bound to NEMSIS-defined `ValueSet` or standard code systems (LOINC, SNOMED CT, RxNorm, ICD-10-CM)

For full element-level mapping, see the [NEMSIS-to-FHIR Mapping](mapping.html) page.
