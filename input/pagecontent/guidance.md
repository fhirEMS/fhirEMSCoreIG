...
| NEMSIS NV Code | FHIR Representation |
|---|---|
...
| NEMSIS PN Code | Meaning | FHIR Representation |
|---|---|---|
...
| NEMSIS Type | FHIR Mapping | Conversion Required |
|---|---|---|
...
2. **Second:** Use `Patient.identifier[ssn]` (tokenized) for matching — never plain-text SSN
3. **Third:** Use demographic matching (name + DOB + address) via MPI
4. **Fourth:** If no match found, create a new patient record and flag for manual review
