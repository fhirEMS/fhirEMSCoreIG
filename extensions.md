# Extensions - FHIR EMS Core Implementation Guide (Unofficial Research Project) v0.2.0
...
1. **Group related extensions**as complex extensions (backbone) to reduce top-level clutter
1. **Bind value sets**from the same NEMSIS CodeSystem used in the parent element
1. **Document NEMSIS source**in the extension's`^comment`using the standard pattern:`NEMSIS eXxx.xx. Usage: level. NV: yes/no. PN: yes/no.`
1. **Alias all extensions**in`aliases.fsh`before referencing in profiles
...
1. Add an alias to`input/fsh/aliases.fsh`
1. Reference in the relevant profile using the alias
1. Document in the Extensions Registry table above
1. Document in`docs/mapping/fhirEMSCore_MappingMatrix.xlsx`→ Extensions_Registry sheet
